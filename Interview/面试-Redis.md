...
- 定期过期：每隔一定的时间，会扫描一定数量的数据库的expires字典中一定数量的key，并清除其中已过期的key。该策略是前两者的一个折中方案。通过调整定时扫描的时间间隔和每次扫描的限定耗时，可以在不同情况下使得CPU和内存资源达到最优的平衡效果。 (expires字典会保存所有设置了过期时间的key的过期时间数据，其中，key是指向键空间中的某个键的指针，value是该键的毫秒精度的UNIX时间戳表示的过期时间。键空间是指该Redis集群中保存的所有键。) ，定期删除会便利所有的database（16），检查指定20个，随机抽查，如果有过期就删除。 
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/14/1717449419419e78~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/14/171744941b1f2a80~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/14/171744941eba6627~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/14/171744941f4fe2e4~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/14/171744945c7745d8~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/14/171744945c8008f8~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/14/1717449461a98262~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
- **redis 没查到，数据库也没查到。**就叫缓存穿透。一般是恶意攻击。

- 解决方法：
  - 在业务层面：进行严格的参数校验。
  - 从缓存取不到的数据，在数据库中也没有取到，这时也可以将key-value对写为key-null，缓存有效时间可以设置短点，如30秒（设置太长会导致正常情况也没法使用）。这样可以防止攻击用户反复用同一个id暴力攻击。
  - 采用布隆过滤器，将所有可能存在的数据哈希到一个足够大的 bitmap 中，一个一定不存在的数据会被这个 bitmap 拦截掉，从而避免了对底层存储系统的查询压力。
    - 布隆过滤器特点：如果一个元素不在集合中，那么他肯定不在；如果在集合中，可能会存在误判。布隆过滤器只能加数据不能减数据。
    - 误判率可以根据哈新散列的程度和提高数组的长度。

#### 什么是缓存击穿？

- **redis 没查到，数据库能查到，就叫缓存击穿。针对一些热点key。**一般出现在数据初始化以及key过期了的情况。
...
- **大量的数据 redis 没查到，数据库能查到，就叫缓存雪崩。** 一般出现在数据初始化以及key同时过期了的情况。
- 解决方法：
