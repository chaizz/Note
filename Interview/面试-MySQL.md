...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c6ee6f5752~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c6ef9cb120~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c6ef8bc6a4~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c6efdfc051~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c6f013b994~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
- 太多索引会影响写性能。
- 生成执行计划，如果索引太多，会降低性能，
- 异常复杂的查询需求，可以使用 ES或其他的方式存储。

组合索引不建议超过5个字段的组合。

不要频繁在更新的字段上创建索引。

非必要不要进行join查询，如果要进行join, join 字段类型必须一样。并建立索引。
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c6f098cc5d~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
所有事务的修改都是线记录到这个undolog 日志中，然后再进行相关的操作。如果执行过程中遇到了异常，可以讲解利用回滚日志中的信息将数据回滚到之前的样子，回滚日志是优先于持久化到磁盘上。



### binlog 有哪几种录入格式，分别有什么区别？

binlog_foramt = STATEMENT(默认)：数据操作时间，每一条数据修改的sql都会记录到binlog,优点是并不需要记录每条数据，和数据变化，减少了日志的数量，节约IO，提高性能，缺点是某些情况下会导致主从不一致，或者是使用函数记录时间等日期不一致。

binlog_foramt = ROW：记录的是数据被修改了，以及被修改什么样了。不会发生上面的时间的问题，但是会产生大量的日志。

binlog_foramt = MIXED：以上两种方式的混合，sql语句有函数时，使用ROW,没有函数时，使用STATEMENT，但是无法识别系统变量。



### mysql 集群同步数据为什么使用binLog, 优缺点是什么？

- 所有引擎都可以使用。
- 支持增量同步数据。
- binlog 可以被其他的中间件读取，例如 hdfs。
- 如果不使用binlog。怎会出现如下的问题：
  - 不支持某个阶段的回放
  - 直接复制的过程中，一旦中断复制，很难确定复制的offset。
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c720eda1ef~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c7211423d0~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
- all：全表扫描
- index: 索引查询
- range: 检索给定范围的行，使用一个索引来选择。
- ref： 使用索引查询。
- eq_ref:使用唯一索引。
- const/ system: 使用常量查询，一些mysql内部的查询。
...
rows:预估扫描的行。采样统计的方法，选择n个页，统计这些页上的不同值，得到这个平均值，然后乘以页数。
...
- using where: 没有使用索引，做条件索引（需要优化）

- using index condition： 没有覆盖索引，但是查询条件使用了索引列。

- using temporary: 需要创建临时表来完成操作，性能较差，建议优化。

- using filesort 使用了文件排序。

- select tables optimized away  使用了聚合函数索引列。
...
子查询虽然更加的灵活，但是效率不如join高，因为在子查询的时候会创建临时表，查询完毕后再删除临时表，子查询慢就是因为多了一个和创建和销毁临时表的过程。join不会创建临时表，所以快一点。

### join 的 查询算法 

如果连接的表关联的数据是有 索引的 会使用 NLJ （Nested Loop join） 查询算法。

没有的花会使用 BNLJ （Buffer Nested Loop join）。

表连接的个数为61。

但是不建议超过三张表join。关联过多会占用太多的内存空间。性能下降。
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c7259992ab~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c725b21e8e~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c7300b465e~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c744498a9a~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
- **跨分片的排序分页问题**
  ...
  ![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c750f5b2cc~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
...
![在这里插入图片描述](https://p1-jj.byteimg.com/tos-cn-i-t2oaga2asx/gold-user-assets/2020/4/13/171735c75eb7e749~tplv-t2oaga2asx-jj-mark:3024:0:0:0:q75.awebp)
